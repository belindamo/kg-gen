    ...
    def deduplicate(self) -> tuple[Graph, dict]:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump({
                ...
                "entity_clusters": {k: list(v) for k, v in deduped_kg.entity_clusters.items()} if deduped_kg.entity_clusters else None,
                "edge_clusters": {k: list(v) for k, v in deduped_kg.edge_clusters.items()} if deduped_kg.edge_clusters else None,
